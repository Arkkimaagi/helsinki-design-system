...
HDS <UsagePageAnchorLink>Oidc client</UsagePageAnchorLink> stores data in the session storage for security reasons. Session storage is not shared across browser windows or tabs, so the user must log in separately for each window.
